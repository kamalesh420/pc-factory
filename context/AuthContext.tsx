...
interface AuthContextType {
    ...
    loading: boolean;
    signup: (email: string, password: string, additionalData?: object) => Promise<any>;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const signup = async (email: string, password: string, additionalData: object = {}) => {
        ...
        // Create user document in Firestore with enhanced fields
        try {
            await setDoc(doc(db, 'users', userCredential.user.uid), {
                email: email,
                uid: userCredential.user.uid,
                role: 'customer', // Default role for new users
                savedBuilds: [], // Array to store saved build IDs
                phone: '',
                address: null,
